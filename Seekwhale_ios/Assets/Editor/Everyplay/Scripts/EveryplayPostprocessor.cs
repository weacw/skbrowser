...
public static class EveryplayPostprocessor
{
    ...
    private static void FixAndroidDynamicLibraryCopyWithGradle(bool enabled)
    {
        ...
        string pluginsEveryplayPath = Path.Combine(pluginsPath, "Everyplay");
        string everyplayAndroidLibraryPath = Path.Combine(Path.Combine(Path.Combine(pluginsPath, "Android"), "everyplay"), "libs");
        string everyplayAndroidLibraryPathArmv7 = Path.Combine(everyplayAndroidLibraryPath, "armeabi-v7a");
        string everyplayAndroidLibraryPathX86 = Path.Combine(everyplayAndroidLibraryPath, "x86");

        if (Directory.Exists(pluginsEveryplayPath))
        {
            string androidPath = Path.Combine(pluginsEveryplayPath, "AndroidDynamicLibraries");
            ...
            if (Directory.Exists(everyplayAndroidLibraryPathArmv7))
            {
                ...
                string dstPath = Path.Combine(androidPath, name);
    ...
    private static void FixAndroidSubprojectBuildGradle(string path)
    {
        ...
        {
            string everyplayDstFolder = Path.Combine(Path.Combine(path, PlayerSettings.productName), "everyplay");
            string dstGradleFile = Path.Combine(everyplayDstFolder, kGradleFile);
            string everyplaySrcFolder = Path.Combine(Application.dataPath, "Plugins/Android/everyplay/");
            string srcGradleFile = Path.Combine(everyplaySrcFolder, kGradleFile);
